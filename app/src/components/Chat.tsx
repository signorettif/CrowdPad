...
export const Chat = ({ messages, onlineCount }: ChatProps) => {
    return (
        <div className="flex-1 bg-white rounded-lg shadow-lg p-6 flex flex-col">
            <h2 className="text-lg font-bold mb-4 text-gray-800">Live Chat</h2>
            <div className="flex-1 overflow-y-auto border border-gray-200 rounded-lg p-4 mb-4 bg-gray-50">
                {messages.length === 0 ? (
                    <div className="text-gray-500 text-center">Messages will appear here...</div>
                ) : (
            ...
            <div className="text-sm text-gray-600 mb-2">
                <span className="font-semibold">Online:</span> <span>{onlineCount}</span>
            </div>
        </div>
    );
};